...
type LocationFilter = LocationNoOp | Location String
type SearchFilter = SearchNoOp | Search String
type TeacherFilter = TeacherNoOp | Teacher User
...
initialModel : Model
initialModel = { filterByLocation = LocationNoOp, filterBySearch = SearchNoOp, filterByTeacher = TeacherNoOp }
...
filterByLocation filter topic =
  let filterTeachingMove teachingMove =
        case filter of
          Location locationName -> teachingMove.location_in_topic == locationName
          LocationNoOp -> True
      teachingMoves = List.filter filterTeachingMove topic.teaching_moves
  in
      { topic | teaching_moves = teachingMoves }

containsValue : String -> String -> Bool
containsValue searchTerm container =
  let lowerContainer = container |> toLower
      lowerTerm = searchTerm |> toLower
  in lowerTerm `contains` lowerContainer

filterBySearch : SearchFilter -> Topic -> Topic
filterBySearch filter topic =
  let filterTeachingMove teachingMove =
        case filter of
          Search search -> List.any (containsValue search) [teachingMove.item.name, teachingMove.item.description]
          SearchNoOp -> True
      teachingMoves = List.filter filterTeachingMove topic.teaching_moves
  in
      { topic | teaching_moves = teachingMoves }
...
filterByTeacher filter topic =
  let filterTeachingMove teachingMove =
        case filter of
          Teacher user -> teachingMove.item.user == user
          TeacherNoOp -> True
      teachingMoves = List.filter filterTeachingMove topic.teaching_moves
  in
      { topic | teaching_moves = teachingMoves }
